...
The one worth naming again: `test_sides_of_different_colours_are_not_bridged` kills the `axis=1`→`axis=2` mutant on the row-median. `axis=1` is the row's dominant **colour**; `axis=2` throws colour away. A greyscale fixture gives the same number either way — the test uses a blue side and a red side with **identical per-pixel channel medians**, so only the correct axis sees a boundary.
