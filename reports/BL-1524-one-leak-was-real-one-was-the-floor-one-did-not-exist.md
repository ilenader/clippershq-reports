...
**The third class lived in a file another round held, and it is now applied too.** I did not take it on a relay: a third party said it had been released, the registry still showed the claim live, so I refused and asked the owner directly — who then released it and ended the claim.

Before that, I had sent that round the anchored patch, and a peer **corrected my own hedge**: I had written that the guard "may be arming something that never fires". That is true of the Instagram implementation, which is gated on an admission token — and **false on the other platform, where the guard raises whenever the free rules have not been recorded, with no token to satisfy it, and can therefore refuse today.** The patch was stronger than I sold it.

Applied and **driven in the state it landed in**, not out of tree: the guard stops the loop, the dollar cap still stops it, and an unrelated `TypeError` still falls through to the error path. Call census identical. **Located by AST rather than by string — that single block had three distinct line numbers in one evening**, and an exact-text anchor would have failed on the third. Only the local class is named there, deliberately, because the camera module is not imported on that path; a test pins that asymmetry so nobody "fixes" it into an `ImportError`.
...
4. **The boundary guard on the other platform — NOW APPLIED, and this is done.** A third party relayed that the holding round had released the file. **The registry still showed the claim live, so I refused to touch it** and asked the owner directly; they then released it, committed, and ended the claim. A relayed intention is not a released claim. **On that platform the guard is falsiness-driven** — it raises whenever the free rules have not been recorded, with no admission token to satisfy it — and is called immediately before each of two billed calls, so **it could refuse today and its refusal was being swallowed while the loop kept buying.** Driven in place: guard → STOPPED, cap → STOPPED, an unrelated `TypeError` → still the error path. Call census identical. **Located by AST, not by string — that one block had three distinct line numbers in a single evening.**
