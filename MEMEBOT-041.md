...
```python
def test_sides_of_different_colours_are_not_bridged(self):
...
- **`L383` `y0 = max(0, int(search_top_frac * frame_h))` → `max(1, …)`.** I wrote `test_a_band_flush_with_the_top_edge_is_found` for this and asserted `band["top"] <= 2`. The mutation shifts the search start by **one pixel**, so my tolerance was wider than the defect. `assertEqual(band["top"], 0)` would have caught it.
- **`L385` `if y1 <= y0: return None` → `<`.** I wrote `test_an_inverted_search_window_returns_none` using fracs 0.9/0.1, which gives `y1 < y0`. The mutation only differs when `y1 == y0`, which my fixture never produces.
